...
mongoose.connect(uri)
  .then(async () => {
    ...
    await generateStudyPlanAsync('st035', 'Student 35', '6a33c6b4d67ba7d81f63916b');
    console.log('Finished generateStudyPlanAsync');
    
    // verify the study plan was saved
    const StudyPlan = require('./models/StudyPlan');
    const plans = await StudyPlan.find({ studentId: 'st035' });
    console.log('Found plans for st035:', plans.length);
    if(plans.length > 0) {
