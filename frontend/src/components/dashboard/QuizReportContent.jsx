...
export default function QuizReportContent() {
    ...
    if (selectedQuizId) {
      fetch(`/api/quiz-results/quiz/${selectedQuizId}?limit=10000`)
        ...
        .then(data => {
          ...
          setCurrentPage(1); // Reset page on new quiz
        })
  ...
  const gradeData = [
    { name: '(90-100)', count: grades.A, color: '#34d399' },
    { name: '(80-89)', count: grades.B, color: '#6ee7b7' },
    { name: '(70-79)', count: grades.C, color: '#93c5fd' },
    { name: '(60-69)', count: grades.D, color: '#fcd34d' },
    { name: '(<60)', count: grades.F, color: '#f87171' },
  ];
  ...
  const sortedSubmissions = [...filteredSubmissions].sort((a, b) => {
    if (sortConfig.key === 'percentage') {
      return sortConfig.direction === 'asc' ? a.percentage - b.percentage : b.percentage - a.percentage;
    } else if (sortConfig.key === 'studentId') {
      if (a.studentId < b.studentId) return sortConfig.direction === 'asc' ? -1 : 1;
      if (a.studentId > b.studentId) return sortConfig.direction === 'asc' ? 1 : -1;
      return 0;
    }
    return 0;
    ...
    <div className="max-w-[1200px] mx-auto font-sans pb-8">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-[0_2px_4px_rgba(0,0,0,0.02)] overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            ...
            <div className="relative w-full sm:w-auto">
              <input
                ...
                className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-indigo-500 focus:ring-[3px] focus:ring-indigo-500/10 transition-all w-full sm:w-[240px]"
              />
        ...
        <div className="p-4 border-t border-slate-100 flex justify-between items-center bg-white text-[13px] text-slate-500 font-medium">
          <span>
            Showing {sortedSubmissions.length > 0 ? ((currentPage - 1) * itemsPerPage) + 1 : 0} to {Math.min(currentPage * itemsPerPage, sortedSubmissions.length)} of {sortedSubmissions.length} entries
          </span>
          <div className="flex gap-1">
            <button 
              onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
              disabled={currentPage === 1}
              className="w-8 h-8 rounded border border-slate-200 flex items-center justify-center hover:bg-slate-50 cursor-pointer disabled:opacity-50"
            >
              &lt;
            </button>
            
            {[...Array(Math.ceil(sortedSubmissions.length / itemsPerPage))].map((_, i) => (
              <button 
                key={i}
                onClick={() => setCurrentPage(i + 1)}
                className={`w-8 h-8 rounded flex items-center justify-center font-bold cursor-pointer ${currentPage === i + 1 ? 'bg-indigo-600 text-white border-indigo-600' : 'border border-slate-200 hover:bg-slate-50 text-slate-600'}`}
              >
                {i + 1}
              </button>
            ))}
            ...
            <button 
              onClick={() => setCurrentPage(p => Math.min(Math.ceil(sortedSubmissions.length / itemsPerPage), p + 1))}
              disabled={currentPage === Math.ceil(sortedSubmissions.length / itemsPerPage) || sortedSubmissions.length === 0}
              className="w-8 h-8 rounded border border-slate-200 flex items-center justify-center hover:bg-slate-50 cursor-pointer disabled:opacity-50"
            >
              &gt;
            </button>
