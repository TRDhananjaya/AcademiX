...
// 2. Fix ESLint error: move fetchStudyPlans ABOVE useEffect
const targetFetch = `  useEffect(() => {
    fetchStudyPlans();
  }, []);
...
const replacementFetch = `  const fetchStudyPlans = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/study-plans', {
        headers: {
...
// 3. Add handleRecoverMissingPlans function
...
// 4. Add Button and Message to UI
...
// 5. Add the recovery message block
...
// Close the div we opened for flex
const targetCloseUI = `                  </div>
                </div>
              </div>
            )}`;
const replacementCloseUI = `                  </div>
                </div>
              </div>
            )}
            </div>`;
code = code.replace(targetCloseUI, replacementCloseUI);
